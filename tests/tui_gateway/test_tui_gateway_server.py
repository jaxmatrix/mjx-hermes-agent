...
def test_compute_host_interrupt_forwards_when_parent_running_mirror_is_stale(monkeypatch):
    ...
    try:
        ...
        assert response["result"] == {"status": "interrupted", "turn_isolation": True, "was_running": False}
...
def test_prompt_submit_ordinal_space_matches_the_display_projection(monkeypatch):
    ...
    class _FakeDB:
        def replace_messages(self, key, messages, active_only=False, archive_dropped=False, reject_active_turn_lease=False):
            replaced.append((key, list(messages)))

        # No durable rows: the ordinal-only path stays open (an ephemeral
        # session), which is the path whose ordinal SPACE this test pins.
...
def test_prompt_submit_truncate_ordinal_skips_a_legacy_recovery_note(monkeypatch):
    ...
    class _StubDb:
        ...
        def replace_messages(self, session_id, messages, active_only=False, archive_dropped=False, reject_active_turn_lease=False):
            self.replaced.append((session_id, list(messages)))

        # No durable rows: keeps the ordinal-only path open for this test.
        def get_messages_as_conversation(self, *_args, **_kwargs):
            return []
...
def test_persist_branch_seed_keeps_reasoning_fields(monkeypatch, tmp_path):
    ...
    try:
        ...
        # role=user row: `user_originated_turn_view` counts it and the projection
        # paints it as a user bubble carrying a restore-checkpoint affordance.
        marker = _branched_marker(db, "branch-key")
        assert marker["display_kind"] == "personality_switch"
