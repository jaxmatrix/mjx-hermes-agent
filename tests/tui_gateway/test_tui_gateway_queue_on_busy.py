...
#
# `prompt.submit` used to rebind the session transport up front, before it knew
# the prompt would only be QUEUED — so viewer B typing while viewer A streamed
# redirected A's in-flight answer to B. Sessions now fan out to every attached
# transport (`session_transports._attach_session_transport`), so the property
# pinned here is membership: the running viewer keeps its stream AND the
# arriving viewer receives it.
...
def test_queued_prompt_does_not_steal_the_running_viewers_stream(monkeypatch):
    ...
    assert server._session_transport_contains(session, viewer_a)
    assert server._session_transport_contains(session, viewer_b)
...
def test_steer_claims_the_stream_it_just_changed(monkeypatch):
    ...
    server._sessions["sid"] = session
    try:
        resp = _submit_as(viewer_b, "r1", {"session_id": "sid", "text": "actually, B"})
    finally:
        server._sessions.pop("sid", None)
