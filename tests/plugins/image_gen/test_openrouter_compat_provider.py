...
class TestProviderClass:
    def test_capabilities_accept_custom_models(self):
        ...
        for provider in _build_providers():
            assert provider.capabilities()["accepts_custom_model"] is True
...
class TestLiveCatalog:
    ...
    def test_picker_merges_image_api_and_chat_catalogs(self):
        ...
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _build_providers
        ...
        assert ids[0] == DEFAULT_MODEL                     # default first
        assert "bytedance-seed/seedream-4.5" in ids        # Image-API-only model present
        assert "openai/gpt-5.4-image-2" in ids             # chat-catalog model present
...
class TestGenerate:
    ...
    def test_chat_chain_falls_back_on_an_access_gate(self):
        """MJXHRM-434: an access-gated head of the chain must advance to the
        next model, not fail the call.

        The guard is the retry gate in the chat path. It must key off
        :func:`_should_try_next`, NOT off ``_access_error_hint`` — that helper
        returns ``None`` for anything outside ``openai/``, and the default
        chain is ``google/``, so hint-gating silently reduces the chain to a
        single attempt. Both the status (403) and the model (non-``openai/``)
        below are chosen so only the correct gate lets this pass.
        """
        import requests as req_lib
        ...
        assert not DEFAULT_MODEL.startswith("openai/"), (
            "the default chain must not need per-account OpenAI image enablement"
        )

        gated = MagicMock()
        gated.status_code = 403
        gated.text = "Forbidden"
        gated.json.return_value = {"error": {"message": "model unavailable"}}
        ...
        with patch(_RUNTIME, return_value=_runtime_ok()), \
             ...
             patch("requests.post", side_effect=[gated, _mock_chat_response(["data:image/png;base64,QQ=="])]) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            result = _openrouter().generate(prompt="a pet")
...
class TestImageApiSurface:
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        # Image-API-*only* models are the ones Nous cannot serve. The default
        # chain is pinned to chat-completions by _CHAT_ONLY_MODELS even though
        # the flash tier also appears in the Image API catalog, so Nous offering
        # it is correct — it reaches it over chat-completions like any other.
        image_api_only = set(_IMAGE_API_MODELS) - _CHAT_ONLY_MODELS
        assert image_api_only, "the Image API catalog must not collapse into the chat chain"
        assert not (image_api_only & nous_ids)
