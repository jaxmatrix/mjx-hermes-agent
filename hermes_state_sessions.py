...
# One conversation's whole compression chain, from any id in it: ancestors reached through parents that
# ended in 'compression', descendants through children of such parents. Identical shape to the CTE in
# :meth:`SessionSessionsMixin._set_lineage_column`, which flips pin/archive/read state across the chain
# as a unit for exactly the same reason — see :func:`_collect_compression_lineage_ids`.
...
def _collect_compression_lineage_ids(conn, session_id: str) -> List[str]:
    ...
    Auto-compression ends the row with ``end_reason='compression'`` and starts a fresh one carrying the
    summary, and every list endpoint surfaces the chain as ONE row (projected onto its live tip). Deleting
    only that row left the root — still holding the pre-compression transcript and its ``pinned`` flag —
    to come BACK into the list as a conversation of its own (MJXHRM-414). Branch children are deliberately
    NOT included: ``/branch`` forks a separate conversation listed in its own right.
    """
    ...
    if session_id not in ids:  # the seed is in the CTE base case; never rely on that for correctness
...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def delete_session(
        ...
        ``include_compression_lineage=True`` deletes the whole compression chain *session_id* belongs to —
        what "delete this conversation" means for any caller that got the id out of a list, since those
        project a chain onto its live tip as ONE row and the orphaned root otherwise resurfaced as a
        conversation of its own (MJXHRM-414). Branch children are still orphaned, not deleted."""
        ...
        def _do(conn):
            if conn.execute("SELECT 1 FROM sessions WHERE id = ? LIMIT 1", (session_id,)).fetchone() is None:
                return False
            target_ids = (
                _collect_compression_lineage_ids(conn, session_id) if include_compression_lineage
                else [session_id]
            )
            if expected_ids is not None and expected_ids != {
                *target_ids, *_collect_delegate_child_ids(conn, target_ids)
            }:
                return False
            removed_ids.extend(_delete_delegate_children(conn, target_ids))
            for chunk in _id_chunks(target_ids):
                ph = _session_ids_placeholders(chunk)
    ...
    def delete_sessions(
        ...
        ``include_compression_lineage=True`` takes each selected conversation's whole compression chain
        (see :meth:`delete_session`, MJXHRM-414). The count still reflects SELECTED conversations, not
        rows, so a caller's "deleted N" keeps meaning what the user picked."""
    ...
    # Pinned rows are exempt: pin is the durable "keep" flag, spared by bulk prune/archive too, so "Delete
    # empty" must not destroy a conversation the user asked to keep.
    _EMPTY_SESSION_WHERE = (
        "message_count = 0 AND ended_at IS NOT NULL AND archived = 0 AND COALESCE(pinned, 0) = 0 "
        "AND NOT EXISTS (SELECT 1 FROM messages WHERE messages.session_id = sessions.id)"
    )

    def count_empty_sessions(self) -> int:
        """Count of empty, ended, non-archived, non-pinned sessions; ended_at guards a fresh session's first
        message."""
    ...
    def delete_empty_sessions(self, sessions_dir: Optional[Path] = None) -> int:
        """Delete every empty, ended, non-archived, non-pinned session in one transaction, orphaning (not cascading)
