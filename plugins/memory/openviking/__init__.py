...
def _local_server_command() -> list[str]:
    ...
    """
    # Env read goes through the profile secret scope like every other OPENVIKING_* read
    # (upstream dropped the raw _env_value/_first_nonempty helpers this used).
    env = (get_secret("OPENVIKING_SERVER_COMMAND") or "").strip()
    raw = env or _clean_config_value(_load_hermes_openviking_config().get("server_command")) or _DEFAULT_SERVER_COMMAND
    try:
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    try:
        ...
        with log_path.open("ab") as log_file:
            subprocess.Popen([server_cmd, *command[1:], "--host", host, "--port", str(port)], stdout=log_file,
                             stderr=log_file, stdin=subprocess.DEVNULL, start_new_session=True, env=child_env)
    except Exception as e:
