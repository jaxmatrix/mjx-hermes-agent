...
# Quality-first default chain: highest fidelity first, the faster tier when it is gated /
# unavailable / times out. Both are routable without per-account enablement — unlike the
# access-gated ``openai/`` image models that used to head this chain, which failed outright on
# most accounts (MJXHRM-434). Any explicit override is exact — no auto fallback.
...
# Chat-path statuses worth re-attempting on the next chain model: 402/403/404 are the access-gate
# shapes (model not enabled / not routable / unknown here); 408/429 and 5xx are transient.
_RETRYABLE_STATUSES = frozenset({402, 403, 404, 408, 429})
# Messages OpenRouter returns for "this model isn't available to you", across assorted statuses.
_RETRYABLE_MESSAGES = ("no endpoints", "not a valid model", "data policy")


def _should_try_next(status: int, err_msg: str) -> bool:
    """Whether a chat-path HTTP failure should advance to the next candidate model.

    Deliberately separate from :func:`_access_error_hint`, which only shapes the *message*: that
    helper is scoped to ``openai/`` models, so gating the retry on it silently disabled fallback
    for every other vendor's chain — including the ``google/`` defaults (MJXHRM-434). ``400``
    (malformed body) and ``401`` (bad credential) fail identically on every model in the chain,
    so retrying them only doubles the latency before the same error surfaces.
    """
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        # These endpoints route any model id the account can reach and the catalog moves faster
        # than we ship, so the pickers let the user type an id we've never heard of.
        return {"modalities": ["text", "image"], "max_reference_images": max_refs, "accepts_custom_model": True}
    ...
    def _generate_via_chat(
        ...
        if failure is not None:
            ...
            # The retry decision comes from the response, never from the openai/-only hint.
            reason = "unavailable" if _should_try_next(status, message) else None
            if hint:
