...
# image models that previously headed this chain: those need account enablement
# / BYOK, so on most accounts the head of the chain failed with an access gate
# and image generation failed outright (MJXHRM-434).
#
...
def _should_try_next(status: int, err_msg: str) -> bool:
    ...
    — including the ``google/`` defaults this backend now ships (MJXHRM-434).
