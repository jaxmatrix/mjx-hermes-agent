...
class _ChildRun:
    ...
    def finish_failed(
        ...
        _safe_progress(
            ...
            # Fork (MJXHRM-410): a steer that won the race with this failure has no boundary left; the parent's
            # tool result names it, and a UI only ever sees this event, so it carries the same fact.
            **({"missed_steer": late_steer} if late_steer else {}),
        )
    ...
    def emit_complete(self, result: Dict[str, Any], entry: Dict[str, Any], duration: float) -> None:
        ...
        # Fork (MJXHRM-410 / MJXHRM-459): facts that live on the parent MODEL's tool-result entry but that a UI
        # has no other channel for — the event stream is all a UI gets.
        # ``missed_steer``: a steer queued after the final assistant turn never landed; the user was told
        # "queued" and must be able to learn it did not.
        _missed_steer = entry.get("missed_steer")
        if isinstance(_missed_steer, str) and _missed_steer.strip():
            complete_kwargs["missed_steer"] = _missed_steer
        # ``truncated``: a child that exhausted its iteration budget still reports ``completed``.
        complete_kwargs["truncated"] = bool(entry.get("truncated"))
        # ``budget_wrapup``: past 80% of ``agent.run_budget_seconds`` the child was told to wrap up — it ran
        # out of TIME, not work. Dormant (key absent) when no budget is configured.
        if getattr(child, "_run_budget_wrapup_injected", False):
            complete_kwargs["budget_wrapup"] = True
        # ``worktree``: the finalize report, the only thing saying whether the child's branch holds anything to
        # review. Requires ``attach_worktree`` to run BEFORE this event (see ``_run_single_child``).
