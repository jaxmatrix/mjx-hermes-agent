...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if configured in ("fal", NOUS_MANAGED_PROVIDER):
        # Explicit FAL keeps the legacy FAL path; "nous" (managed Nous
        # Subscription selection) also runs the legacy pipeline, which routes
        # through the managed fal-queue gateway.
        return None
    auto = not configured
    if auto:
