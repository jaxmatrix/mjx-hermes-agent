...
def _auto_selected_provider():
    ...
    Only ever consulted when *nothing* is stored, so it can never override a
    stored selection — the strict-selection rule owns that case.
    ...
    pipeline rather than the plugin dispatch path (see issue #26241). Note the
    name ``"nous"`` is overloaded — as a *stored* value it is the managed Nous
    Subscription selection (NOUS_MANAGED_PROVIDER, handled before we get here),
    while as a *resolved* provider it is the Nous Portal image backend, which
    dispatches through the plugin path like any other.
    """
    try:
...
def _dispatch_to_plugin_provider(
    ...
    (``"fal"`` / ``"nous"``, or unset with no auto-selected backend). Providers without ``upscale`` ignore it via ``**kwargs``."""
    configured = _read_configured_image_provider()
    if configured in ("fal", NOUS_MANAGED_PROVIDER):
        return None  # explicit FAL / managed Nous keep the legacy in-tree pipeline
    auto = not configured
    if auto:
        auto_provider = _auto_selected_provider()
        if auto_provider is None:
            return None  # nothing resolvable — legacy FAL fall-through
        configured = auto_provider.name
    # Not for an auto-selected backend: image_gen.model is written alongside
    # image_gen.provider, so with no provider set any top-level value belongs to a
    # different backend — and plugins treat an explicit model kwarg as "use exactly
    # this", which would also disable their fallback chain.
...
def _active_image_capabilities() -> Dict[str, Any]:
    ...
    Mirrors runtime dispatch: a set ``image_gen.provider`` asks that plugin; unset asks
    whatever :func:`_auto_selected_provider` resolves; otherwise the FAL catalog.
