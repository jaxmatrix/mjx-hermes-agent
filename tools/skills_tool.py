...
# Thread-local, not a module global (fork 045d9b4364). The gateway wires this per turn with a callback closed
# over that turn's session id, and turns for different sessions run concurrently on their own threads — as a
# module global the LAST turn to wire won, so a secret prompt raised by session A was emitted carrying session
# B's id while A's agent stayed blocked. Mirrors the thread-local sudo callback in tools/terminal_tool.py. The
# module-level default stays under its original name for non-gateway callers and existing monkeypatches.
