...
def steer_subagent(
    ...
    Thin wrapper over :func:`steer_subagent_reason` — a caller that has to tell a USER why the steer did not
    land wants that one, because every refusal above collapses into the same ``False`` here.
    """
    return steer_subagent_reason(
        subagent_id, text, owner_session_id=owner_session_id, owner_transport=owner_transport,
        owner_session_record=owner_session_record,
    ) == STEER_QUEUED


# The subagent-steer outcome vocabulary. ``queued`` is the only acceptance; every other value names a DIFFERENT
# refusal, and they do not mean the same thing to whoever sent the text: "already past its last step" is a race
# they lost, "not yours" is a wiring bug, "no longer running" means the work is already over. ``subagent.steer``
# returns the exact string as ``reason`` beside ``status: "rejected"`` (tui_gateway/methods_session.py).
STEER_QUEUED = "queued"
...
def steer_subagent_reason(
    ...
    ``"queued"`` on acceptance, else ``"empty_text"``, ``"unknown_subagent"`` (never registered, or already
    unregistered), ``"not_accepting"`` (registered but past its last delivery boundary —
    ``_close_subagent_steering`` has already run), ``"not_owner"`` (the caller's session/transport/record
    identity is not the one this child was commissioned under), ``"no_agent"`` (a record with no live agent)
    or ``"steer_failed"`` (the child's own ``steer()`` refused or raised).
    """
