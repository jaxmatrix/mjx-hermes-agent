...
from tools.delegate_tool_registry import (  # noqa: F401
    ...
    steer_subagent,
    # Fork (MJXHRM-410): the named-refusal steer API ``subagent.steer`` returns as ``reason``.
    STEER_EMPTY_TEXT, STEER_FAILED, STEER_NO_AGENT, STEER_NOT_ACCEPTING, STEER_NOT_OWNER, STEER_QUEUED,
    STEER_UNKNOWN_SUBAGENT, steer_subagent_reason,
)
...
def _run_single_child(
    ...
    * ``"completed"``       — normal finish. See #97655.
    ...
    try:
        ...
        # Finalize the worktree BEFORE the completion event so its report rides on it (fork f0c96e4265).
        run.attach_worktree(entry)
