...
class InterruptControlMixin:
    ...
    def redirect(self, text: str) -> bool:
        ...
        Thin boolean wrapper over :meth:`redirect_outcome` — every existing surface (CLI, TUI, gateway
        busy-mode, ACP) asks only "did you take it?". A surface that has to TELL THE USER what happened wants
        ``redirect_outcome``: ``True`` here covers both a real rebuild-the-turn redirect and the tool-boundary
        degrade, which look nothing alike on screen."""
    ...
    def redirect_outcome(self, text: str) -> str:
        ...
        ``"redirected"`` — the live model request was cancelled and the turn is being rebuilt with the
        correction folded in (or Codex's native ``turn/steer`` took it); the reply on screen is superseded.
        ``"steered"`` — a tool was executing, so the correction degraded to :meth:`steer` and rides on the
        NEXT tool result; nothing on screen is superseded and the model has not seen it yet (if the turn ends
        before another tool batch runs, the caller gets it back as ``result["pending_steer"]``).
        ``"rejected"`` — no live turn, empty text, or a hard interrupt already won; the surface must fall
        back to its next-turn queue.

        The split exists because ``redirect()`` answered ``True`` for the middle case, so ``session.redirect``
        reported ``redirected`` for a correction that had changed nothing yet (MJXHRM-410 / MJXHRM-80).
        """
        if not text or not text.strip():
            return "rejected"
        cleaned = text.strip()
        ...
        if _native_steer is not None:
            with _ic_lock(self, "_pending_redirect_lock"):
                if self._interrupt_requested:
                    return "rejected"
            try:
                # Native active-turn steering: the running turn itself consumes the correction, so this is a
                # redirect, not the deferred tool-boundary degrade below.
        ...
        if getattr(self, "_executing_tools", False):
            ...
            if accepted:
                ...
                if tracker is not None and tracker_lock is not None:
                    ...
                    for tid in worker_tids:
                        _request_yield(tid)
            return "steered" if accepted else "rejected"

        _model_active = getattr(self, "_model_request_active", None)
        with _ic_lock(self, "_pending_redirect_lock"):
            if _model_active is None or not _model_active.is_set():
                return "rejected"  # response completed before we got the lock: surface queues a new turn
