/**
 * Universal's session LIST / LIFECYCLE layer — search, paging, pinning,
 ...
 * setters, the owner hints and the workspace-cwd helpers) and is kept
 * byte-identical to `apps/desktop/src/store/session.ts` so every future resync of
 * it is mechanical — an AUTO file for `scripts/desktop-sync.mjs` rather than a
 * hand-merge. Universal's lifecycle layer has no desktop counterpart (desktop
 * spreads the same concerns across controllers and hooks), so it lives here
 * under its own name instead of being folded back in.
 *
 * The state atoms are IMPORTED from `./session`, never redeclared: two atoms for
 * one concept (two `$sessions`, two `$activeSessionId`) is a real bug, not a
 * duplication smell. Where universal's behaviour genuinely exceeded desktop's,
 * the extra lives here as a distinctly-named wrapper over desktop's primitive,
 * with a comment saying what it adds.
 */
...
// Desktop's session STATE store. Everything universal shares with it — the
...
// Keyed by BACKEND SCOPE, not by profile alone (MJXHRM-591): stored ids are
// unique per backend database, so a marker that names only a profile can send
// the next boot to open connection A's id against connection B — against a
// database that never heard of it, or worse a recycled one that names somebody
// else's conversation. `backendScopeKey` keeps the local connection's bare
// profile key, so a single-source install's persisted markers are unchanged.
const $lastSessionByProfile = persistentAtom<Record<string, string>>(LAST_SESSION_KEY, {}, Codecs.stringRecord)

/** Where this profile's place on this connection is remembered. */
const markerScope = (profile: string): string => backendScopeKey($activeConnectionId.get(), profile)
...
if (ownsPersistedAppState()) {
  $activeStoredSessionId.subscribe(id => {
    ...
    const owner = markerScope(knownSessionProfileFor(id) ?? $activeGatewayProfile.get())
...
export function lastOpenedSessionId(): null | string {
  return $lastSessionByProfile.get()[markerScope($activeGatewayProfile.get())] ?? null
}
...
export function forgetLastSessionMarkers(leavingConnectionId?: null | string): void {
  ...
  // Only the connection being left (invariant 37). Its own markers go because
  // the marker is where boot lands and the app is no longer there; the places
  // remembered on every OTHER connection are not this switch's to forget.
  const local = leavingConnectionId === LOCAL_CONNECTION_ID
  const prefix = backendScopePrefix(leavingConnectionId)

  const kept = Object.fromEntries(
    // The local connection's markers keep the BARE profile key, so its entries
    // are the ones that carry no `conn:` prefix at all.
    Object.entries(remembered).filter(([scope]) => (local ? scope.startsWith('conn:') : !scope.startsWith(prefix)))
  )
 ...
 * a moment ago — `knownSessionProfileFor` has no row to read and
 ...
 *
 * Desktop's `knownSessionProfile(sessions, id)` answers the same question from a
 * row list plus its owner hints, and is used verbatim for that rung. What it
 * cannot answer is a session NO listing will ever contain — a plugin's hidden
 * session — which is why `profileByStoredId` (fed by `rememberSessionProfile`
 * and `resolveSessionProfile`) is consulted after it. That second rung is the
 * extra universal carries, so this keeps its own name rather than shadowing
 * desktop's export.
 */
export function knownSessionProfileFor(storedSessionId: string): string | undefined {
  return knownSessionProfileFromRows($sessions.get(), storedSessionId) || profileByStoredId.get(storedSessionId)
}
...
export async function resolveSessionProfile(storedSessionId: null | string): Promise<string | undefined> {
  ...
  const known = knownSessionProfileFor(storedSessionId)
...
//
// The STORE is desktop's `store/session-removal` — `$removedSessionIds`,
// `tombstoneSessions`, `untombstoneSessions` and the in-flight set, imported
// above. Universal had an identical private copy here, but desktop's
// `store/projects.ts` and the resume actuators already read session-removal's
// atom, so two atoms would have meant the tree and the list disagreeing about
// what exists. The read-side helpers below have no session-removal counterpart
// and stay here, now reading the shared atom.
// ---------------------------------------------------------------------------

function markSessionMutation(ids: readonly (null | string | undefined)[], inFlight: boolean): void {
  const pending = ids.filter((id): id is string => Boolean(id?.trim()))

  inFlight ? beginSessionMutation(pending) : endSessionMutation(pending)
}
...
export function pruneSessionTombstones(stillListed: Iterable<string>): void {
  ...
  const pending = new Set([...current].filter(id => listed.has(id) || inFlight.has(id)))
...
/**
 * Drop every "finished while you were away" marker at once — the sidebar filter
 * menu's "Mark all as read".
 *
 * Desktop's `markAllSessionsRead` clears the transient atom and stops there.
 * Universal ALSO has to flush the injected durable layer first, or the next list
 * refresh recomputes every dot the user just dismissed straight back out of the
 * persisted watermarks and markers — so this keeps its own name and delegates
 * the atom write.
 */
export function markAllSessionsReadDurable(): void {
 ...
 * (auto-compression mints a new session id mid-turn). Guarded by provenance so a
 * stale background rotation can't steal the foreground selection. Called by
 * `session-states.ts#handleTransition`.
 *
 * NOT desktop's `setActiveSessionStoredIdRotation`, which is the plain setter
 * for the `$activeSessionStoredIdRotation` one-shot EVENT atom that desktop's
 * route-following effect consumes. Universal has no such effect — it applies the
 * rotation to the selection here and now — so the two are different operations
 * and keep different names.
 */
export function applyActiveSessionStoredIdRotation(rotation: {
...
async function reclaimWarmSession(
  ...
  // hit on this answer.
  if (!knownSessionProfileFor(storedId) && sessionProfileIsAmbiguous()) {
...
async function hydrateColdSession(storedId: string): Promise<void> {
  ...
  const known = knownSessionProfileFor(storedId)
 ...
 * dedicated session-picker overlay. Universal has no such overlay — the
 * equivalent surface is the Command Center's `sessions` section — so this routes
 * there instead of flipping desktop's `$sessionPickerOpen` atom, which nothing
 * in universal renders. Closing is the route's own job (Esc / backdrop), so
 * `false` is a no-op. Desktop's setter is still re-exported under its own name
 * from `@/store/session` for the ported code that drives the atom.
 */
export function openSessionPickerRoute(open: boolean): void {
  if (open) {
...
export async function renameSessionLocal(id: string, title: string): Promise<void> {
  ...
  try {
    await renameSession(target, title, knownSessionProfileFor(target))
  } catch (err) {
    $sessions.set(prev)
...
export async function deleteSessionLocal(id: string): Promise<void> {
  ...
  const owner = knownSessionProfileFor(id)
