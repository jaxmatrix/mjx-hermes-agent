...
import { getGlobalModelOptions, setEnvVar, setModelAssignment, startOAuthLogin, validateProviderCredential } from '@/hermes'
...
const oauthProvider = (flow: 'device_code' | 'pkce'): OAuthProvider =>
  ({ id: 'anthropic', name: 'Anthropic', flow, cli_command: '', docs_url: '', status: { logged_in: false } }) as OAuthProvider
...
describe('onboarding store', () => {
  ...
  it('enters the OAuth step for a device_code provider', async () => {
    startLogin.mockResolvedValueOnce({ flow: 'device_code', session_id: 'sess', verification_url: 'https://v', user_code: 'ABCD', expires_in: 600, poll_interval: 5 } as never)
  ...
  it('completes a PKCE flow: submit code → confirm step', async () => {
    startLogin.mockResolvedValueOnce({ flow: 'pkce', session_id: 'sess', auth_url: 'https://a', expires_in: 600 } as never)
