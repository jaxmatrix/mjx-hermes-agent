...
const { isUrlServer, shouldNotifyOnTransition } = await import('./mcp-health')
...
describe('isUrlServer — what the background sweep is allowed to probe', () => {
  it('probes an enabled HTTP/SSE server', () => {
    expect(isUrlServer({ url: 'https://mcp.linear.app/mcp' })).toBe(true)
    expect(isUrlServer({ url: 'https://mcp.linear.app/mcp', enabled: true })).toBe(true)
  })

  // The whole point of the guard: probing a stdio server SPAWNS the command,
  // so a half-hourly background timer must never touch one — enabled or not.
  it('never probes a stdio server, even an enabled one', () => {
    expect(isUrlServer({ command: 'npx', args: ['-y', 'server-fs'], enabled: true })).toBe(false)
    expect(isUrlServer({ command: 'docker' })).toBe(false)
  })

  it('skips a disabled server and a malformed entry', () => {
    expect(isUrlServer({ url: 'https://mcp.linear.app/mcp', enabled: false })).toBe(false)
    expect(isUrlServer({ url: 42 })).toBe(false)
