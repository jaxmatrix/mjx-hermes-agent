/**
 * LEGACY — universal's session-key route dispatch; retires with the old fold
 * (MJXHRM-602 fold step 6). Desktop's owner-route surface and its one-argument
 * `sessionRpcNeedsProfileRoute` live in `store/session-request-router`.
 *
 ...
 */
export function legacyRouteNeedsProfileParam(
...
const singleGatewayRouter: SessionRequestRouter = {
  ...
  dispatch<T>(route: SessionRoute, method: string, params: Record<string, unknown>, timeoutMs?: number): Promise<T> {
    ...
    const scoped = route.scopeProfile && legacyRouteNeedsProfileParam(route.profile, active)
