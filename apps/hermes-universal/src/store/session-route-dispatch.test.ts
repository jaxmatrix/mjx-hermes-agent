...
} = await import('./session-route-dispatch')
...
describe('legacyRouteNeedsProfileParam', () => {
  it('routes ambient for a blank owner and for the live profile, pins anything else', () => {
    expect(legacyRouteNeedsProfileParam(undefined, 'default')).toBe(false)
    expect(legacyRouteNeedsProfileParam('  ', 'default')).toBe(false)
    expect(legacyRouteNeedsProfileParam('default', 'default')).toBe(false)
    expect(legacyRouteNeedsProfileParam('work', 'default')).toBe(true)
  })
})
