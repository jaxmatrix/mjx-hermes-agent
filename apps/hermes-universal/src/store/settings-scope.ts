...
// Safety, Memory & Context, Voice, Tools & Keys) and the Messaging page, so
...
// keeps single-profile users on the exact pre-existing code path (requests fall
// back to the app-wide active profile in hermes.ts `profileScoped`).
//
// Ported verbatim from apps/desktop/src/store/settings-scope.ts; universal's
// `@/store/profile` already exports the two primitives it is built on.
...
// previous target. Same drop-the-override contract as `useOnProfileSwitch`.
