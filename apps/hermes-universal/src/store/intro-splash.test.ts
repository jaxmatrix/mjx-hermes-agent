...
describe('$introSplash', () => {
  it('defaults ON, matching desktop', () => {
    ...
    // fallback a first-run device sees — `persistBoolean` then writes it back
    ...
    expect(window.localStorage.getItem('hermes.desktop.intro-splash.v1')).toBe('true')
  })
  ...
  it('persists the user turning it off', () => {
    ...
    expect(window.localStorage.getItem('hermes.desktop.intro-splash.v1')).toBe('false')
  })
})
