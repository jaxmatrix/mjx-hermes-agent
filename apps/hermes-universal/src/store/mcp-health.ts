...
/** Whether the background sweep may probe this server at all.
 *
 *  Exported for the test that pins it: a stdio entry passing this check would
 *  make a half-hourly timer SPAWN a user-configured local command. */
export const isUrlServer = (server: Record<string, unknown>): boolean =>
 ...
 *
 * Refuses in a satellite/tile/activity window: `App()` mounts in every window
 * this app opens, so an ungated start would give the fleet one sweeper PER
 * WINDOW — several processes probing the same servers on the same cadence and
 * several copies of the same nudge.
 */
export function startMcpHealthChecker(): void {
  if (started || typeof window === 'undefined' || isSecondaryWindow() || isActivityWindow()) {
    return
  }
