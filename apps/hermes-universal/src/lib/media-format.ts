...
// Pure, store-free media path/kind helpers. Kept separate from lib/media.ts so
// modules that only need formatting (e.g. lib/chat-media's MEDIA: rewriting,
// imported into store/chat + session-history) don't drag in media.ts's
// connection/transport chain — importing any symbol from a module loads the
// whole module, and media.ts pulls @/store/connection + @/lib/desktop-fs.
