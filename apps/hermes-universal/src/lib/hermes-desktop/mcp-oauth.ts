...
const listen: NonNullable<Bridge['mcpOauth']>['listen'] = async () =>
  invokeNative('mcp_oauth_listen')

const wait: NonNullable<Bridge['mcpOauth']>['wait'] = async (id, timeoutMs) =>
  invokeNative('mcp_oauth_wait', { id, timeoutMs })

const cancel: NonNullable<Bridge['mcpOauth']>['cancel'] = async id =>
  invokeNative('mcp_oauth_cancel', { id })
