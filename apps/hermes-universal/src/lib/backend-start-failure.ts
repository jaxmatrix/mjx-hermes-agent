 ...
 *
 * VENDORED VERBATIM from `apps/desktop/electron/backend-start-failure.ts`
 * (MJXHRM-446). Universal declares no dependency on the desktop workspace and
 * `vite.config.ts` aliases only `@` and `@hermes/plugin-sdk`, so a shared
 * predicate is copied rather than imported — the same reason
 * `lib/backend-scope.ts` is a copy. Keep the five predicates byte-identical:
 * they encode failures both apps learned the hard way, and a divergence here is
 * a retry loop nobody notices until a bundle shows 157 of them.
 *
 * WHERE THE WORDS DIFFER: desktop says "startHermes()" and "the renderer's retry
 * loop"; universal's equivalents are `dialSavedTarget` / `selectConnection` and
 * `runReconnectLoop` (`store/connection.ts`). The latch itself lives in
 * `store/connection-latches.ts`, keyed per CONNECTION rather than one global
 * `backendStartFailure` — with a registry, one dead source must not stand the
 * supervisor down for the others.
 *
