// Ported verbatim from apps/desktop/src/lib/runtime-readiness.ts. Derives an
// inference-readiness verdict ({ready, reason}) from the gateway's `setup.status`
// + `setup.runtime_check` RPCs, so the statusbar gateway-health indicator can say
// ready / needs-setup / checking. Same gateway backend, so the RPCs are the same.
