...
export function DecodedLabel({ leaving, text }: { leaving?: boolean; text: string }) {
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className="dither ms-1.5 -mr-[0.875rem] inline-block size-2 shrink-0 -translate-y-px rounded-[1px] text-primary decode-cursor-blink"
      />
    </span>
  )
}
...
export function HackeryButton({
  ...
  return (
    ...
    >
      ...
      <span className="-mr-[0.25em] ps-[0.25em] tracking-[0.25em]">{label}</span>
      <span className="text-primary/40 transition-colors group-hover:text-primary">]</span>
    </button>
  )
}
