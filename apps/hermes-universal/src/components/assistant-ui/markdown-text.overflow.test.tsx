...
// size exceeded" crash loop, ported from desktop fa92aa1984.
//
...
//
// The throw escaping to the app's root boundary is what blanks the whole
// window, so what matters is that every markdown surface stays up and keeps the
// text readable. These mount the real production component; a boundary that
// silently stopped being applied has to fail here.
afterEach(cleanup)
...
describe('markdown surface survives stack-overflow content', () => {
  ...
  // three render through this component.
  it.each([
