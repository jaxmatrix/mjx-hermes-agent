 ...
 *
 * Ported from apps/desktop/src/components/assistant-ui/transcript-directive.tsx.
 * The one behavioural addition is the `isSettledDirective` gate — see the
 * comment on that function for why a brace-less directive must not render
 * mid-stream.
 */
...
export const TranscriptDirectiveLeaf: FC<{ text: string; streaming?: boolean }> = ({ text, streaming }) => {
  ...
  const settled = parsed !== null && isSettledDirective(parsed, streaming ?? false)
  const match = settled && parsed ? claimFor(contributions, parsed.name) : undefined
...
export function useIsClaimedDirective(text: string | null, streaming = false): boolean {
  const contributions = useContributions(TRANSCRIPT_DIRECTIVE_AREA)
  const parsed = text === null ? null : parseTranscriptDirective(text)

  return parsed !== null && isSettledDirective(parsed, streaming) && claimFor(contributions, parsed.name) !== undefined
}
