 ...
 *  window is born at.
 *
 *  80 since the composer became two rows (input above, attach + model + voice
 *  controls below) with uniform 0.5rem padding. The bar measures 76px at the
 *  HUD's 600px width, which buckets to 80 — so this is what the first measured
 *  report asks for, which is the property the Rust constant's comment is asking
 *  us to preserve. Born smaller, the HUD would paint a clipped second row until
 *  the first resize landed. */
