...
import { $petMotion, $petRoamDir, type PetMotion } from '@/store/pet'
...
// Ported from apps/desktop/src/components/pet/use-pet-roam.ts (only the store
// import differs — universal publishes PetMotion, not the desktop PetState).
 ...
 * walks along surfaces (the window floor, the top of a perch, …), hops up onto
 * higher ledges, and drops off them — instead of drifting through empty space.
 * Walkable surfaces come from `roam-geometry` (re-measured from the live DOM
 * every beat) and the beat-to-beat choices from `roam-behavior`; this hook owns
 * only the physics and the DOM writes.
 *
 ...
 * committed (via `commit`) when the pet settles. Two signals publish the wander:
 * `$petMotion` (`run`/`jump`) flips the sprite pose, and `$petRoamDir` (-1/0/1)
 * picks the directional run row + mirror.
 */
export function usePetRoam({
  ...
  useEffect(() => {
    ...
    // One chokepoint for the wander signals: the pose (drives the sprite) and the
    // travel direction (drives the directional row + mirror).
    const signal = (pose: PetMotion | null, dir: -1 | 0 | 1) => {
      $petMotion.set(pose)
      $petRoamDir.set(dir)
    }
