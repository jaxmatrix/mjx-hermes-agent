 ...
 * live DOM for walkable surfaces and answers pure questions about them. Ported
 * from apps/desktop/src/components/pet/roam-geometry.ts — the only change is that
 * the overlay inset is derived from the `--titlebar-height` CSS var (universal)
 * instead of a `TITLEBAR_HEIGHT` constant.
 */
...
// floor is always a ledge; these add app furniture the pet can climb onto.
// (Universal doesn't tag these yet — they simply no-op until the data-slots
// exist; add a `data-slot` here to grow the playground.)
