...
// Per-scope cache key. The BASE key (no suffix) is the app-wide active profile,
// unchanged for every caller that passes nothing. An explicit scope — the
// settings "Applies to" selector editing ANOTHER profile — gets its own
// suffixed key, so switching the selector refetches and never paints one
// profile's config while claiming to edit another's.
...
export const useHermesConfigRecord = (profile?: null | string) =>
  useQuery({
    ...
    // null and undefined both mean "no override" → fetch with undefined so the
    // REST layer falls back to the app-wide active profile.
...
// write the suffixed per-profile cache instead — keeps a scoped page's
// optimistic write-through landing on the same key its own query reads.
