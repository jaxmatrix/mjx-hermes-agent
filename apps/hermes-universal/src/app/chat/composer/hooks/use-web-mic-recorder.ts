...
/**
 * Webview microphone recorder (`getUserMedia` + `MediaRecorder`).
 *
 * This is the ORIGINAL path, kept for mobile (where it works) and as a desktop
 * fallback. It is unreliable on Linux/WebKitGTK — see `useNativeMicRecorder` for
 * the Rust capture path that replaces it on desktop.
 */
export function useWebMicRecorder(copy: MicRecorderErrorCopy): MicRecorder {
