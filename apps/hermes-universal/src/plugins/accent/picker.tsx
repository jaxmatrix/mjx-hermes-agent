// The accent picker, IN-TREE (`src/plugins/*`) rather than in the shared
// sample-plugin directory: `packages/hermes-sample-plugins/` is the ONE canonical
// copy both apps glob (rule 30), and an app's own plugin is not a sample. This is
 ...
 *
 *  Over the app's own `startPointerDrag` rather than desktop's hand-rolled pair
 *  of window listeners: universal runs on touch, where the platform can steal a
 *  pointer mid-gesture and deliver `pointercancel` INSTEAD of `pointerup` — and
 *  a mouse-only teardown leaks a live `pointermove` handler every time it does. */
function useDrag(onMove: (x: number, y: number) => void) {
  ...
  const onPointerDown = useCallback((event: React.PointerEvent) => {
    ...
    stopRef.current?.()
    stopRef.current = startPointerDrag(e => emit(e.clientX, e.clientY))
  }, [])

  // The drag cannot know its owner unmounted mid-gesture — the popover closing
  // while the pointer is down is exactly that.
