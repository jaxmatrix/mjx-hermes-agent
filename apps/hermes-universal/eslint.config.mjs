...
const HAND_WRITTEN_CSS_CLASSES =
  /(^|:)(arc-(border|composer|nous|reverse|row)|aui-(error-action|md|md-table|prose-fence|shiki)|bg-warning\/15|blur-0|brand|chat|codicon(-modifier-spin)?|composer-(bars|no-drag)|connect(-body|-card|-head|-step-head|-title|-welcome)?|decode-cursor-blink|local-install(-bar(-fill)?|-block|-log|-path|-stage|-stages|-status)?|desktop-input-chrome|dither|fit-text|glyph-spinner(__frame|__strip)?|hud-native-drag|katex-host|not-prose|pane-tab-content|particle(-field|__glyph|__sway)?|pet-(contact-shadow|egg-shadow(--sm)?|reveal)|preview-(markdown|source-code)|progress-slide|quest-glow|reaction-pop|ref|rs-root|scrollbar-overlay|session-import|shiki|shimmer(-color-primary)?|status-(artifacts|control-row|dismiss-button|drawer(-clip|-content|-toggle)?|row(-actions|-content|-dismiss|-icon)?|section-(body|header|icon|trigger)|subagent-detail)|sticky-human-clamp|tab-key-hint-icon|text-warning|thinking-preview|thread-(jump-button|timeline-(tick|ticks|track))|titlebar-icon-button|tool-(group-scroll(--faded)?|memory-legendary-(glyph|meta|title)|ticker(__reel|__row)?)|tooltip-bubble(-label)?|wake-indicator(-light|-surface)?|wordmark)$/
