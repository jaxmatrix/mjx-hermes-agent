...
def _resume_response(
    ...
    # A scheduled continuation makes inflight/running/status above wrong: a turn is starting on this session
    # and the crash marker holds its prompt (fork b605ea9a09).
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params, writer=True) as db:
        ...
        try:
            ...
            # The whole compression chain: the session list projects a chain onto its live tip and returns it
            # as ONE row, so deleting only that row left the root behind — and with no tip to project onto it
            # reappeared as a conversation of its own (MJXHRM-414). Branches keep their own rows.
            deleted = db.delete_session(target, sessions_dir=home / "sessions", include_compression_lineage=True)
        except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    sid = str(params.get("session_id") or "")
    # Whether there WAS a live turn is the only thing separating a Stop that did something from one that did
    # nothing; reported so a client can settle a turn whose terminal frame died with a socket (MJXHRM-366).
    ...
    if _session_uses_compute_host(session):
        ...
        return _ok(rid, {"status": "interrupted", "turn_isolation": True, "was_running": was_running})
    # Stop NEVER waits on the agent (fork 17941739b6). Re-resolving through `_sess` starts the deferred build
    # and blocks on `agent_ready`, so a first turn parked behind a cold build — the moment Stop matters most —
    # hung and then failed, while `_turn_cancel_requested` (the actual cancel for a parked turn) was never set.
    # `_interrupt_session_turn` sets that flag and tolerates a missing agent (request_hard_interrupt(None)).
    ...
    return _ok(rid, {"status": "interrupted", "was_running": was_running})
...
def _apply_correction(rid, session: dict, verb: str, text: str, accepted_status: str) -> dict:
    ...
    try:
        if verb == "redirect":
            # redirected / steered / rejected (fork e310e313ba, 492e95e068): a correction deferred to the next
            # tool boundary has superseded nothing yet, and a client places its bubble by THIS value.
            accepted_status = _agent_redirect_outcome(session["agent"], text)
            accepted = accepted_status != "rejected"
        else:
...
def _(rid, params: dict) -> dict:
    ...
    is not "delivered": a child past its final tool batch surfaces ``missed_steer`` on the parent entry AND on
    the ``subagent.complete`` event. A refusal is a 200 ``rejected`` with a ``reason`` naming which one
    (``steer_subagent_reason``), plus this handler's ``no_session_authority`` for an unverified caller."""
    ...
    result = {"status": "queued" if queued else "rejected", "subagent_id": subagent_id, "text": text}
    if not queued:
