...
def _set_focus(rid, params, key, value, session):
    ...
    # display_only (fork bc0396229b): for clients that hide rows THEMSELVES at render time. Pinning tool_progress
    # stops tool.start/tool.complete for the session — right for a scrollback TUI, wrong for a GUI whose todo
    # panel, changed-files card and images feed off those events — so record the shared flag, keep the data.
    display_only = bool(params.get("display_only", False))
    stash = d_f.get("focus_saved_tool_progress")
    if display_only:
        effective = _load_tool_progress_mode()
    elif target:
        saved = (cur_focus and stash) or _load_tool_progress_mode()
        ...
        # Nothing was stashed (focus enabled display-only, or by hand): restore none rather than a hardcoded
        # "all" that silently overwrote a user on `new` or `verbose`.
        effective = _load_tool_progress_mode()
    ...
    # display_only echoed so a client can tell an honouring gateway from one that pinned tool progress anyway.
    return _kv(rid, key, "on" if target else "off", tool_progress=effective, display_only=display_only)
