...
class PluginEnvRow(Result):
    """``hermes_cli/plugins_cmd.py::plugin_env_rows`` — one ``requires_env`` / ``optional_env`` entry."""
...
class AgentPluginRow(Result):
    ...
    env: list[PluginEnvRow] = Field(default_factory=list)
