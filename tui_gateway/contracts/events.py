...
class SteerMissedPayload(Payload):
    """``prompt_turn._run_post_turn_followups`` — a /steer the ended turn never saw (it is requeued next)."""

    text: str


event("steer.missed", SteerMissedPayload,
      doc="A steer accepted mid-turn was never delivered before the turn ended; it is requeued as a prompt.")
...
class SubagentEventPayload(Payload):
    ...
    truncated: bool | None = None  # the child hit its iteration budget; its summary is partial
    budget_wrapup: bool | None = None  # the child was told to wrap up at the run-budget threshold
    worktree: dict[str, JsonValue] | None = None  # ``tools/subagent_worktree.py`` finalize report
    missed_steer: str | None = None  # steer text accepted as ``queued`` that never reached the child
