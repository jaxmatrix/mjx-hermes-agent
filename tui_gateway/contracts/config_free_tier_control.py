...
class ConfigSetParams(ProfileParams):
    ...
    display_only: bool = False  # ``focus`` only: record the flag but keep tool events flowing (client hides rows)
...
class ConfigSetResult(Result):
    ...
    tool_progress: str | None = None
    display_only: bool | None = None  # ``focus`` echoes it so a client can tell an honouring gateway
    cwd: str | None = None
    branch: str | None = None
