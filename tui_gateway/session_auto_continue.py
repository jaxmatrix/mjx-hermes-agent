...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        try:
            ...
            with session["history_lock"]:
                # The turn's finally never runs on this path and _session_live_status reads the flag as
                # "working": a dispatch that blew up must not report a turn forever (fork b605ea9a09).
                session["_auto_continue_scheduled"] = False
    ...
    # ``prompt`` is internal: the only copy of the interrupted request a COLD resume has (a hard crash persists
    # nothing of the running turn). _apply_auto_continue_resume_state pops it into ``inflight`` (fork b605ea9a09).
    return {"attempt": attempt, "interrupted_at": marker["started_at"], "prompt": marker["prompt"]}
...
def _ac_try_correction(rid, session: dict, agent: Any, method: str, plain_text: str, status: str) -> dict | None:
    ...
    try:
        if method == "redirect":
            # Name what happened (fork 492e95e068): a redirect that degraded to a deferred tool-boundary steer
            # answers "steered", the same value the explicit steer mode returns — reporting it as "redirected"
            # put the correction bubble above a reply it had not touched (MJXHRM-410 / MJXHRM-80).
            outcome = _agent_redirect_outcome(agent, plain_text)
            if outcome == "rejected":
                return None
            status = outcome
        elif not getattr(agent, method)(plain_text):
            return None
    except Exception:
        return None
    with session["history_lock"]:
