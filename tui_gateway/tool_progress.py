...
_SUBAGENT_FIELDS = (
    ...
    # Fork (MJXHRM-459 / MJXHRM-410): this builder is an allow-list, so a completion kwarg delegate_tool sets
    # reaches no client unless listed. cost_usd feeds the Agents-overlay spend header; truncated = the child hit
    # its iteration budget (its "completed" summary is partial); budget_wrapup = it was told to wrap up at 80%
    # of agent.run_budget_seconds; worktree = the finalize report under delegation.worktree_isolation;
    # missed_steer = a steer accepted as "queued" that never reached the child.
    ("cost_usd", _not_none, _float_or_skip), ("truncated", _not_none, bool), ("budget_wrapup", bool, bool),
    ("worktree", lambda v: isinstance(v, dict), dict), ("missed_steer", bool, str),
)
