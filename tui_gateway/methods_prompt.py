...
def _history_user_indices(history: list) -> list:
    """Indices of the user turns a client can SEE — THE ordinal space.

    Delegates to server.py's ``_counts_as_user_ordinal`` (resolved through the
    rebound globals) rather than re-deriving ``role == "user" and not
    display_kind`` here: that predicate mirrors EVERY way ``_history_to_messages``
    drops a role="user" row — stored ``display_kind``, the legacy sniff for
    crash-recovery notes, ``[System: …]`` bookkeeping markers, rows that render
    empty. The inline rule honored only the first, so a session carrying a
    personality-switch marker rewound one turn early, silently, for good.
    """
    return [i for i, m in enumerate(history) if _counts_as_user_ordinal(m)]
