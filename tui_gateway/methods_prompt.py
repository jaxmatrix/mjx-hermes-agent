...
def _history_user_indices(history: list) -> list:
    """Indices of the user turns a client renders as user bubbles, including composite carriers.

    THE ordinal space for ``truncate_before_user_ordinal``. A client counts the user bubbles
    ``_history_to_messages`` handed it and sends that index, and the cut it drives is a
    destructive ``replace_messages()``. Every row counted here that the projection does not
    render as a user bubble shifts each later ordinal by one and silently drops a turn the user
    never pointed at. ``user_originated_turn_view`` still accepts untyped legacy rows the
    projection hides (``[System: …]`` bookkeeping markers) or re-types as a timeline event (the
    crash-recovery note), so those two clauses stay here (c1b41d8e1f, d2cfa2c1be).
    """
    from agent.context_compressor import user_originated_turn_view
    from tui_gateway.session_history import _coerce_message_text, _is_display_hidden_marker, _legacy_display_kind
    ...
    for i, message in enumerate(history):
        ...
        if view is None:
            continue
        text = _coerce_message_text(view.get("content"))
        if _is_display_hidden_marker("user", text) or _legacy_display_kind("user", text):
            continue
        indices.append(i)
    return indices
...
def _(rid, params: dict) -> dict:
    ...
    while True:
        with session["history_lock"]:
            ...
            if has_truncation:
                # A REWIND is not a mid-turn prompt and must never be folded into the busy path: steering
                # hands the text to the very turn the rewind exists to discard, and queueing runs it later
                # with the truncation silently dropped — either answers _ok, so a client that already cut
                # its transcript optimistically only learns on the next hydration that every "deleted" turn
                # came back. 4009 is what both clients already retry on (interrupt, then resubmit).
                return _err(rid, 4009, "session busy — interrupt the current turn before rewinding it")
