...
def _agent_redirect_outcome(agent, text: str) -> str:
    ...
    ``redirect()`` answers a bare ``True`` both for a real redirect (the model request is cancelled and the
    turn rebuilt around the correction) and for the tool-execution degrade, where it silently becomes
    ``steer()`` and rides on the next tool result. Reporting the second as ``redirected`` put a correction
    bubble above a reply it had not touched (MJXHRM-410 / MJXHRM-80). Runtimes that predate
    ``redirect_outcome`` answer exactly what they did before.
    """
...
def _apply_auto_continue_resume_state(payload: dict, auto_continue: dict | None) -> dict:
    ...
    A cold resume registers a brand-new record, so every cold branch reports ``inflight: None`` / idle — honest
    until ``_maybe_schedule_auto_continue`` schedules a continuation, from which moment a turn IS about to run
    and the marker holds the only copy of its prompt. Without this a client resumed to an idle-looking chat,
    sealed what it had recovered as a finished reply, and learned of the turn only at ``message.start`` — up to
    the 120s agent build later. ``inflight.streaming`` is the field both GUI clients key "turn is live" off.
    The descriptor's ``prompt`` is internal and popped here; it never reaches the wire as a descriptor key.
    """
    ...
    if prompt:
        payload.update(inflight={"assistant": "", "streaming": True, "user": prompt}, running=True,
                       status="streaming")
    return payload
...
def _session_live_status(sid: str, session: dict) -> str:
    ...
    if session.get("running"):
        return "working"
    # A scheduled auto-continue owns the session from _maybe_schedule_auto_continue returning until its kickoff
    # flips ``running`` — a window as long as the agent build. "idle" across it contradicts the resume payload
    # that just said the turn is live and darkens the row in session.active_list pollers (fork b605ea9a09).
    return "working" if session.get("_auto_continue_scheduled") else "idle"
