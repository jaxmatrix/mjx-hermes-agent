...
def _canonical_session_row(db, profile_path):
    """``(summary, looked)`` for the profile's canonical "Bot Chat" row (identity is the NAME).
    ...
    The two-tuple is the fork contract apps/hermes-universal reads (MJXHRM-517): ``(None, True)`` means
    "looked, and this profile has no Bot Chat"; ``(None, False)`` means "could not look" (no/unreadable
    state.db, a storage error). Collapsing the second into the first tells a client the chat is gone when
    the read merely failed, and a client that believes that mints a replacement — forking the bot's
    memory. So a failure degrades to SILENCE (the caller omits the key), never to a false negative.
    """
    if db is None:
        return None, False
    try:
        ...
        return {
            ...
            "message_count": tip_row.get("message_count") or 0}, True
    except Exception:
        return None, False
...
def _profile_session_fields(row, profile_path, preferred_id=None):
    """Attach last_session / worker_session / canonical_session (+ preferred_session for a pinned id) to a
    roster row. The DB is a read-only attach (a writable ``SessionDB()`` waits up to 20s for the write lock +
    runs DDL and stalled the 5s roster poll); no/unreadable DB -> last/worker None and ``canonical_session``
    OMITTED ("could not look", never a false "no Bot Chat" — see ``_canonical_session_row``)."""
    ...
    try:
        ...
        if preferred_id:
            row["preferred_session"] = _preferred_session_row(db, preferred_id)
    finally:
        if db is not None:
...
def _(rid, params: dict) -> dict:
    ...
    # session, so clients must not append it to SOUL.md. canonical_session_title discloses the capability:
    # with it present, an absent per-profile ``canonical_session`` means THAT profile's lookup failed, not
    # that the gateway is too old to resolve one.
    return _ok(rid, {"profiles": out, "bot_mode_protocol": True, "canonical_session_title": BOT_CHAT_TITLE})
