...
def _admit_prompt_turn(
    ...
    with session["history_lock"]:
        ...
        if not isinstance(inflight, dict) or inflight.get("status") == "error":
            # An auto-continue submits a wrapper NOTE, not the user's words; the snapshot is what a resuming
            # client paints as the user bubble, so record the interrupted REQUEST (set by the kickoff thread,
            # popped later by _record_turn_marker) — fork b605ea9a09.
...
def _run_post_turn_followups(
    ...
    if isinstance(steer, str) and steer.strip():
        # Tell the surface BEFORE requeueing: the turn the correction was aimed at ended without seeing it, and a
        # client that painted it above the live reply on a steered/redirected answer has no other way to learn
        # that (MJXHRM-410 / MJXHRM-80, fork 492e95e068).
        _emit("steer.missed", sid, {"text": steer})
        with session["history_lock"]:
