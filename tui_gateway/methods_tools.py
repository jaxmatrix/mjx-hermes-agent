...
def _(rid, params: dict) -> dict:
    ...
    try:
        # The session's OWN working directory, not the gateway's launch directory: /diff resolves its
        # repository from TERMINAL_CWD, so a GUI session pointed at another project used to get the launch
        # directory's changes back with no indication they were the wrong ones.
        payload = {"output": worker.run(cmd, cwd=_session_cwd(session)) or "(no output)"}
...
def _plugin_rows() -> list[dict]:
    ...
    for name, version, desc, source, _dir, key in sorted(pc._discover_all_plugins()):
        ...
        out.append({
            ...
            # Fork: the manifest's requires_env/optional_env with an is_set flag, so a client can offer the
            # key field next to the switch instead of sending the user to hunt for it on the Keys page.
            "env": pc.plugin_env_rows(pc._read_manifest(_dir_path)) if _dir_path else [],
