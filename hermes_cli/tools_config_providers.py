...
# Sentinel row appended to model pickers whose backend routes arbitrary ids (mirrors the vision
# backend picker's free-text escape hatch).
_CUSTOM_MODEL_ROW = "Type a custom model id…"


def image_gen_accepts_custom_model(plugin_name: str) -> bool:
    """Whether image gen *plugin_name* routes model ids outside its own catalog (``accepts_custom_model``
    capability). True for backends whose provider catalog moves faster than we ship (OpenRouter, Nous
    Portal); False for closed id sets, where a typed id only produces a runtime failure later."""
    registry = _plugin_registry("agent.image_gen_registry")
    try:
        provider = registry.get_provider(plugin_name)
        return bool(provider is not None and (provider.capabilities() or {}).get("accepts_custom_model"))
    except Exception:
        return False


def _write_plugin_image_model(image_gen_cfg: dict, plugin_name: str, model_id: str) -> None:
    """Persist a chosen image model to both ``image_gen.<plugin>.model`` and ``image_gen.model``. The
    deepinfra and xai plugins resolve only the scoped key, so a top-level-only write was a silent no-op
    for them; other backends and the GUI catalog endpoint still read the top-level key."""
...
def _pick_model_from_catalog(
    ...
    scoped_key: Optional[str] = None, accepts_custom: bool = False,
) -> None:
    """Column-aligned model picker shared by the FAL, plugin image gen and video gen flows.
    Writes the choice to ``config[cfg_key]["model"]`` (and ``config[cfg_key][scoped_key]["model"]`` when
    *scoped_key* is given, whose value is also preferred as the current model). The current model is listed
    first so the cursor lands on it; on a closed-set backend a saved model belonging to another provider
    (shared config key) or a drifted catalog default never indexes the catalog. *accepts_custom* keeps an
    off-catalog id visibly selected and appends a free-text row. Safe when stdin is not a TTY —
    curses_radiolist keeps the current selection."""
    from hermes_cli.tools_config import _cfg_section, _prompt, _prompt_choice

    # An empty catalog configures nothing — unless the backend accepts a typed id.
    if not catalog and not accepts_custom:
        return
    cur_cfg = _cfg_section(config, cfg_key)
    scoped = cur_cfg.get(scoped_key) if scoped_key else None
    ...
    if current_model and current_model not in catalog:
        # A hand-entered id lives outside the catalog; keep it listed so it doesn't appear to revert.
        catalog = {**catalog, current_model: {"strengths": "custom"}}

    model_ids = list(catalog.keys())
    ordered = [m for m in [current_model] + [m for m in model_ids if m != current_model] if m]
    widths = {
    ...
    if accepts_custom and idx >= len(ordered):
        ...
        chosen = ordered[idx]
    if scoped_key:
        _write_plugin_image_model(cur_cfg, scoped_key, chosen)
    else:
...
def _configure_gen_model_for_plugin(section: str, plugin_name: str, config: dict) -> None:
    ...
    if section == "image_gen":
        # Resolved through tools_config (like _prompt_choice) so its patch targets stay effective.
        from hermes_cli import tools_config

        catalog, default_model = tools_config._plugin_image_gen_catalog(plugin_name)
        _pick_model_from_catalog(catalog, default_model, section, plugin_name, config, scoped_key=plugin_name,
                                 accepts_custom=tools_config.image_gen_accepts_custom_model(plugin_name))
        return
    catalog, default_model = _plugin_video_gen_catalog(plugin_name)
    _pick_model_from_catalog(catalog, default_model, section, plugin_name, config, row_indent="  ")
