...
def config_floor_refused(current_version: int, latest_version: int) -> bool:
    ...
    calls this and nothing else. Missing/unparseable files never trip the gate.
    """
    ...
    return bool(has_explicit_version and below_support_floor(current_version, latest_version))
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from hermes_cli.config_migrations import (
        run_migrations, support_floor_message)
...
TERMINAL_CONFIG_ENV_MAP = {
    ...
    # Fork (88467eb624): dashboard/universal shell-pane kill switch, read by hermes_cli/web_server.py.
    "shell_pty": "TERMINAL_SHELL_PTY",
    # NOTE: shell_pty_backend / allow_unsandboxed_shell are deliberately NOT bridged here
    # (fork 6293dd1046 / 16e15dc9f4). Only keys present in the raw config.yaml override env, so
    # the old "defaults clobber container env" argument no longer applies; the choice stands because
    #   1. the bridge buys nothing — both are read only in-process by web_server.py's
    #      _resolve_shell_pty_backend / the allow_unsandboxed_shell gate, env first, config second;
    #   2. bridging them would INVERT that precedence: an explicit config.yaml entry would start
    #      beating the deployment's TERMINAL_SHELL_PTY_BACKEND / TERMINAL_ALLOW_UNSANDBOXED_SHELL.
    #      allow_unsandboxed_shell opens an unsandboxed shell to anyone holding a session token, so
    #      that flip is a security regression, not a precedence preference.
    # Fork (8492f01669): cli.py and gateway/run.py have always bridged home_mode; this map did not,
    # so `terminal.home_mode` silently had no effect under `hermes serve` and the dashboard backend.
...
def manifest_env_is_secret(name: str, meta: Optional[Dict[str, Any]] = None) -> bool:
    """Whether a plugin.yaml env declaration is a password field.

    Explicit ``password``/``secret`` wins; otherwise anything named *TOKEN,
    *SECRET, *KEY, *PASSWORD or *JSON is treated as a secret. Shared with
    ``plugins_cmd.plugin_env_rows`` so the Keys page and plugin rows agree.
    """
    meta = meta or {}
    if meta.get("password") is False:
        return False
    if meta.get("password") or meta.get("secret"):
        return True
    return name.upper().endswith(("_TOKEN", "_SECRET", "_KEY", "_PASSWORD", "_JSON"))


def _bundled_plugin_manifests():
    """Yield ``(category_dir, dir_name, manifest_dict)`` for every bundled
    ``plugins/<category>/<name>/plugin.y(a)ml``."""
    plugins_root = get_project_root() / "plugins"
    if not plugins_root.is_dir():
        return
    for manifest_path in sorted(plugins_root.glob("*/*/plugin.y*ml")):
        ...
        try:
            ...
            continue
        yield manifest_path.parent.parent.name, manifest_path.parent.name, manifest


def _inject_plugin_env_vars() -> None:
    """Populate OPTIONAL_ENV_VARS from bundled plugin manifests so their vars are configurable in
    ``hermes config`` UI / the Keys page without the core knowing they exist.
