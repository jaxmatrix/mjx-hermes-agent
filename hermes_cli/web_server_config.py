...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    # Must stay in step with tools/tool_backend_helpers.py::_VALID_MODAL_MODES — coerce_modal_mode
    # silently discards anything else, so ["sandbox", "function"] both collapsed to "auto".
    "terminal.modal_mode": _select(
        "Modal sandbox mode. auto picks per workload; direct runs in this process; "
        "managed uses a Modal-side runner.",
        "auto", "direct", "managed",
    ),
    "terminal.shell_pty": _select("Dashboard shell terminal: auto = enabled, off = disabled", "auto", "off"),
    "terminal.shell_pty_backend": _select(
        "Where the shell pane runs. auto = inherit terminal.backend; set it explicitly to keep "
        "agent execution sandboxed while the pane shells into the host.",
        "auto", "local", "docker", "ssh",
    ),
    "terminal.allow_unsandboxed_shell": {
        "type": "boolean",
        "description": (
            "Permit the unsandboxed host shell when the dashboard is bound to a non-loopback "
            "address. Anyone with a valid session then has host code execution as the gateway "
            "user — only enable on a trusted, auth-gated network."
        ),
        "category": "security",
    },
    "proxy.enabled": {
