...
# /api/shell-pty helpers (route lives in web_routers/chat_ws.py); re-exported so
# ``web_server.<name>`` stays the late-binding seam (tools' backend picker uses it).
from hermes_cli.web_server_shell_pty import (  # noqa: E402,F401
    _effective_terminal_backend,
    _normalize_shell_pty_mode,
    _resolve_shell_pty_backend,
    _shell_pty_allow_unsandboxed,
    _shell_pty_disabled_reason,
    _shell_pty_env,
    _shell_pty_target,
    _warn_shell_pty_bind,
)
...
def start_server(
    ...
    # Tell the operator what /api/shell-pty will do on this bind (refuse vs.
    # unsandboxed host shell) — same resolver as the route, so they cannot disagree.
    _warn_shell_pty_bind(host)
