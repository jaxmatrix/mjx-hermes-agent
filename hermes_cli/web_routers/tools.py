...
async def get_toolset_models(
    ...
    def _read():
        ...
        return {
            ...
            # Drives the GUI's free-text model field; closed-set backends stay list-only.
            "accepts_custom_model": custom_ok}
...
async def select_toolset_model(
    ...
    def _run():
        with config_write_scope(body.profile or profile):
            ...
            section_cfg = _dict_section(config, section)
            if section == "image_gen":
                # Write the scoped key too — deepinfra and xai resolve only
                # image_gen.<plugin>.model, so a top-level-only write made the GUI
                # report success while generation kept the old model. Same write
                # the CLI picker performs.
...
async def get_terminal_backends(profile: Optional[str] = None):
    ...
    error response. ``active`` is the backend the process actually uses,
    ``configured`` what config.yaml says; ``pending`` / ``restart_required``
    mark a selection not yet in force."""
    def _read():
        with _profile_scope(profile):
            config = load_config()
        ...
        return {
            ...
            "restart_required": configured != active, "backends": backends}
