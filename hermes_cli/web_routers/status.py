...
def _config_floor_warning():
    """``{below_floor, support_floor_version}`` for a config too old for the auto-migration
    ladder, so clients stop hardcoding the floor. Structured rather than the prose
    ``support_floor_message()`` (which embeds the HERMES_HOME path; this endpoint is public).
    Best-effort: an unreadable config must never break the liveness probe."""
    try:
        from hermes_cli.config import config_floor_status
        return config_floor_status()
    except Exception:
        return None
...
async def get_health():
    ...
    ``features`` advertises endpoints a client cannot otherwise discover. A WebSocket
    route that does not exist is NOT a 404 on the wire: Starlette closes it pre-accept
    and uvicorn renders any pre-accept close as a bare 403, indistinguishable from an
    auth/origin rejection. So a client dialling ``/api/shell-pty`` against an older
    gateway can only tell "not allowed" from "no terminal API" by asking here first.
    """
    ...
    return {"ok": True, "version": __version__,
            ...
            "features": {"shell_pty": shell_pty, "shell_pty_reattach": shell_pty}}
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        status = {
            ...
            "config_floor_warning": _config_floor_warning(),
