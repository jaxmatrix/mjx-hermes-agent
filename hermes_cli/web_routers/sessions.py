...
def _prune_sessions(body: SessionPrune):
    ...
    try:
        ...
        # Pinned rows are spared unless the caller opted in; report HOW MANY so the client can say so instead
        # of silently under-deleting (mirrors the note `hermes sessions prune` prints). The difference between
        # the two counts is exactly the pinned matches.
        skipped_pinned = 0
        if not body.include_pinned:
            ...
            skipped_pinned = max(
                ...
                - db.count_prune_matches(**_base, include_pinned=False), 0)
        if body.dry_run:
        ...
        return {"ok": True, "removed": removed, "skipped_open": skipped_open, "skipped_pinned": skipped_pinned}
    finally:
        db.close()
...
async def bulk_delete_sessions_endpoint(body: BulkDeleteSessions):
    ...
    # Whole compression chains: the rows the user ticked came out of a list that shows a chain as ONE row, so
    # deleting only that row left the root to reappear as a conversation of its own (MJXHRM-414). The count
    # still reflects selected conversations, not rows.
    deleted = await asyncio.to_thread(
        _with_db, body.profile,
        lambda db: db.delete_sessions(body.ids, include_compression_lineage=True), read_only=False)
...
async def delete_empty_sessions_endpoint(profile: Optional[str] = None):
    ...
    * Active sessions are skipped (``ended_at IS NULL``) so a live agent isn't yanked mid-handshake. *
    Archived sessions are skipped — the user explicitly chose to keep those rows. * Pinned sessions are
    skipped — the pin IS the "keep this" flag, and there is no opt-in here because this endpoint takes no
    filters to narrow the blast radius. * Children of deleted parents are orphaned, not cascade-deleted.
    See #95868.
    """
    deleted = await asyncio.to_thread(
...
async def delete_session_endpoint(session_id: str, profile: Optional[str] = None):
    def _delete(db):
        ...
        # The whole compression chain: every list endpoint projects a chain onto its live tip and shows it as
        # ONE row, so deleting only the tip left the root (and its ``pinned`` flag) to reappear in the list
        # as a conversation of its own (MJXHRM-414). Branches are still orphaned, not deleted.
