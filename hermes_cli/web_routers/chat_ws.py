"""Chat-tab WebSocket routes: /api/console, /api/pty, /api/shell-pty, the /api/ws gateway
...
# --- /api/shell-pty: interactive login-shell PTY over WebSocket. Unlike /api/pty
# (the Hermes TUI) this spawns a shell for the Hermes Universal app's right-pane
# terminal. Same gates/close codes, PtyBridge and keep-alive registry as /api/pty;
# identical wire protocol (raw bytes + the RESIZE escape). Helpers live in
# hermes_cli.web_server_shell_pty; /api/health advertises availability.
_shell_pty_disabled_reason = late("_shell_pty_disabled_reason", "hermes_cli.web_server_shell_pty")
_shell_pty_target = late("_shell_pty_target", "hermes_cli.web_server_shell_pty")


async def _shell_pty_refuse(ws: WebSocket, reason: str) -> None:
    """Refusal = close 4404 with the reason on the close frame too (the pane's end
    panel covers the scrollback). NEVER 1011: the client treats 1011 as "respawn a
    LOCAL host shell", the exact outcome a refusal exists to prevent."""
    await ws.send_text(f"\r\n\x1b[31mTerminal unavailable: {reason}\x1b[0m\r\n")
    await ws.close(code=4404, reason=_ws_close_reason(reason))


@router.websocket("/api/shell-pty")
async def shell_pty_ws(ws: WebSocket) -> None:
    from hermes_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    gate = await _ws_gate(ws, "shell-pty")
    if gate is None:
        return
    peer, mode, _cred = gate
    await ws.accept()
    _log.info("shell-pty accepted peer=%s mode=%s", peer, mode)

    disabled = _shell_pty_disabled_reason()
    if disabled:
        await _shell_pty_refuse(ws, disabled)
        return
    ...
    # Optional (path-hardened) ?cwd= consumed only by the local branch; docker/ssh
    # discard it (container/remote paths differ).
    cwd_param = (ws.query_params.get("cwd") or "").strip() or None
    network_exposed = False
    try:
        from gateway.platforms.base import is_network_accessible

        network_exposed = is_network_accessible(getattr(ws.app.state, "bound_host", "127.0.0.1"))
    except Exception:
        pass
    # Backend probe/discovery shells out to docker info/ps (~2s) — off the event loop.
    ...
    if refusal:
        ...
        await _shell_pty_refuse(ws, refusal)
        return

    # ?attach=<token> opts into keep-alive (the shell outlives this socket and
    # reattaches by token, replaying the ring buffer); no token = ephemeral 1:1 pump.
    ...
    if attach is None:
        try:
            ...
            await _pty_fail(ws, f"Terminal unavailable: {exc}")
            return
        except (FileNotFoundError, OSError) as exc:
            await _pty_fail(ws, f"Terminal failed to start: {exc}")
            return
        await _legacy_pump(ws, bridge)
        return

    # Namespaced so shell-pty tokens can never collide with /api/pty's keys.
    ...
    # A shell repaints from the replayed buffer; no forced TUI redraw needed.
    if not await session.attach(ws):
        await _close_stalled_pty_input(ws, path="shell-keepalive-attach")
        PTY_REGISTRY.detach(reg_key, ws)
        return

    # Writer loop only: the session's drain task forwards output and ring-buffers
    # while detached; on child EOF it closes this socket with 4410.
    try:
        while True:
            try:
                msg = await ws.receive()
            ...
            if match and match.end() == len(raw):
                ...
                continue
            if not await session.write(ws, raw):
                await _close_stalled_pty_input(ws, path="shell-keepalive")
                break
    except WebSocketDisconnect:
        pass
    finally:
        # Detach only — the PTY keeps running for a reattach; the reaper closes it.
        PTY_REGISTRY.detach(reg_key, ws)
