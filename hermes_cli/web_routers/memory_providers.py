...
from plugins.memory.config_schema import (
    STORAGE_CONFIG_YAML, STORAGE_HONCHO_HOST_BLOCK, ProviderConfigSchema, ProviderField, get_provider_config_schema,
)
...
def _write_provider_config_yaml(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    """Persist submitted fields to ``config.yaml`` under ``memory.<provider>`` (the block the
    provider's runtime reads — OpenViking, RetainDB). Secrets go to the env store. A written
    non-secret field also clears its ``env_fallbacks`` from ``.env`` and the process: the
    runtime reads env first, so a stale ``OPENVIKING_ENDPOINT`` would make the save a no-op."""
    config = load_config()
    memory_config = _memory_section(config)
    block = memory_config.get(provider.name)
    if not isinstance(block, dict):
        block = memory_config[provider.name] = {}
    _save_submitted_secrets(provider, values)
    for field in provider.fields:
...
def _declared_provider_payload(provider: ProviderConfigSchema) -> Dict[str, Any]:
    ...
    if is_honcho:
        ...
        if provider.storage == STORAGE_CONFIG_YAML:
            # The runtime resolves env before config.yaml, so an env override must show
            # as the value in effect, not the shadowed stored one.
            overrides = {
                field.key: env[env_key]
                for field in provider.fields if not field.is_secret
                for env_key in field.env_fallbacks if env.get(env_key)
            }
            data_sources = (overrides, _read_config_yaml_block(provider))
        else:
            data_sources = (_read_flat_json(provider),)
...
def _update_memory_provider_config(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    writer = {
        STORAGE_HONCHO_HOST_BLOCK: _write_provider_honcho,
        STORAGE_CONFIG_YAML: _write_provider_config_yaml,
    }.get(provider.storage, _write_provider_flat)
