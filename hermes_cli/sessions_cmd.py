...
def _cmd_delete(db, args):
    ...
    # `hermes sessions list` surfaces a compression chain as one row (its live tip), so the id the user just read
    # names the whole conversation; deleting only that row left the root to reappear in the very next listing
    # (MJXHRM-414). Branches stay: they list in their own right.
    if not db.delete_session(resolved_session_id, sessions_dir=_sessions_dir(), include_compression_lineage=True):
        return _not_found(args.session_id)
