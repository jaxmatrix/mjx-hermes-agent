...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Prompt block that has the model state its working hypothesis, ask a batched handful of
        # questions only when the answer is genuinely the user's to give, and otherwise proceed on a
        # named assumption. Only when `clarify` is loaded and never for kanban workers. ~120 cached
        # tokens. False disables.
...
OPTIONAL_ENV_VARS = {
    ...
    "OPENVIKING_AGENT": _tool("Optional OpenViking peer ID for separate assistant context",
        "OpenViking agent", password=None, advanced=True),
    "OPENVIKING_ACCOUNT": _tool("OpenViking account override for local/trusted mode",
        "OpenViking account", password=None, advanced=True),
    "OPENVIKING_USER": _tool("OpenViking user override for local/trusted mode",
        "OpenViking user", password=None, advanced=True),
    "OPENVIKING_CLI_CONFIG_FILE": _tool(
        "Path to the ovcli.conf profile Hermes links (default: ~/.openviking/ovcli.conf)",
        "OpenViking ovcli.conf path", password=None, advanced=True),
    "OPENVIKING_SERVER_COMMAND": _tool(
        "Command Hermes runs to autostart a local OpenViking server (default: openviking-server)",
        "OpenViking server command", password=None, advanced=True),
